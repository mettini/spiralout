...
vec3 finalize(vec3 col) {
    ...
    float n  = hash21(fc + vec2(s * 137.0, s * 234.0));
    ...
    // v12: dither 16/255 (crossing con 12/255 dio 0.05% YT emulated, intentamos
    // mismo o más para que outbound alcance ese mismo nivel).
    col += vec3(dhR - 0.5, dhG - 0.5, dhB - 0.5) * 16.0 / 255.0;
    // v16: film-grain luma-only adaptativo BUMPEADO. Detector v11 mostró que
    // v15 (2.5→7) dejaba pasar bandas en humo + bloom decline (luma media
    // 0.3-0.5 donde VP9 macroblockea fuerte). Subo piso a 4 y techo a 10.
    // Rotación temporal evita grid pattern.
    float gl = grain_luma(fc, u_time);
    float luma_pre = dot(col, vec3(0.299, 0.587, 0.114));
    float gamp = mix(4.0, 10.0, smoothstep(0.05, 0.7, luma_pre));
...
vec3 render_scene() {
    ...
    // ── FASE 2 (experimento A/B — ver docs/video/27) ──────────────────
    // Atmósfera de BAJA frecuencia espacial (~250-600px en 4K): le da al
    // encoder de YouTube estructura DCT-baja que preservar. Es la única
    // textura que sobrevive al re-encode (el grain per-pixel se borra).
    ...
    float atmo_mod = (atmo1 - 0.5) * 0.62 + (atmo2 - 0.5) * 0.38;
    col *= 1.0 + atmo_mod * 0.1;
    // Apertura de rango tonal: contraste suave alrededor de la media
    // oscura. La zona 4:20-4:40 vivía en Y 60-91 (31 niveles para todo
    // el frame = bandas de 800px post-YouTube).
    float cm = 0.085;
    col = (col - cm) * 1.3 + cm;
...
def render_full(width, height, fps, n_frames, out_mp4, audio_wav, with_audio, duration_s):
    ...
    if out_mp4.exists():
        out_mp4.unlink()
    print(f"[render] {width}x{height} @ {fps}fps, {duration_s}s = {n_frames} frames")
    ...
    if with_audio and audio_wav.exists():
        args += ["-ss", "0", "-t", f"{duration_s}", "-i", str(audio_wav)]
    args += [
    ...
    gl = build_gl(width, height)
    ctrl = load_control(CONTROL_NPZ, n_frames, fps)
    ...
    try:
        for i in range(n_frames):
            ...
            if (i + 1) % 100 == 0 or i == n_frames - 1:
                ...
                print(f"  [{i+1:>5}/{n_frames}] {rate:.2f}fps  elapsed {elapsed/60:.1f}m  eta {eta/60:.1f}m  last100 {dt:.1f}s")
                sys.stdout.flush()
    finally:
