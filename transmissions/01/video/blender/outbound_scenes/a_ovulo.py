...
for f in range(N):
    t=f/24.0
    hb=heart(t)
    # AGRANDADO 1:14-1:17 (74->77). Material hasta 79 para que el xfade con el tunel complete.
    dive=ease((t-74.0)/3.0)
    base=1.0+0.006*hb            # LATIDO sutil: ±0.6% SOLO el ovulo
    grow=1.0+0.6*dive            # agrandado de la transicion (max en 77)
    ...
    beep.outputs[0].default_value=0.13 + 1.4*dive   # glow base tenue; ilumina en el dive
