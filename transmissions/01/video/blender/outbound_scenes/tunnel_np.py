...
def frame(t):
    ...
    v=(0.35/r + t*0.35)                  # profundidad: scroll = volar hacia adelante
    ...
    img+=np.exp(-(r/0.055)**2)*3.0        # nucleo de luz al fondo
    img+=np.exp(-(r/0.16)**2)*0.9         # halo de la luz (se ve el resplandor)
    ...
    return grim(rgb.astype(np.uint8), scale=1.0, grain_amt=2.0, gamma=1.32)
import glob
