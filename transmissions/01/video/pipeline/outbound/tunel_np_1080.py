import numpy as np, sys, os
sys.path.insert(0,'/Users/emilianomettini/git/spiralout/transmissions/01/video/blender')
from grim_post import grim
from PIL import Image
from scipy.ndimage import gaussian_filter
# ===== 1080 nativo: mismo look que el 960 aprobado, escalado por S=2 =====
S=2; W,H=960*S,540*S; cx,cy=W/2,H/2
yy,xx=np.mgrid[0:H,0:W].astype(float)
X0=(xx-cx)/(H/2); Y0=(yy-cy)/(H/2)
# TEXTURA DE PARED tileable (roca organica): noise multi-octava
rng=np.random.default_rng(4); TW,TH=512,512
base=np.zeros((TH,TW))
for oc,(sc,amp) in enumerate([(8,1.0),(16,0.5),(32,0.25),(64,0.12)]):
    n=rng.random((sc,sc)); n=np.asarray(Image.fromarray((n*255).astype(np.uint8)).resize((TW,TH),Image.BICUBIC),float)/255
    base+=n*amp
base=(base-base.min())/(base.max()-base.min())
base=np.vstack([base,base[::-1]]); base=np.hstack([base,base[:,::-1]])  # mirror tile
BTH,BTW=base.shape
def frame(t):
    X=X0; Y=Y0                              # SIN zoom: velocidad constante (solo scroll)
    r=np.sqrt(X*X+Y*Y)+1e-4; th=np.arctan2(Y,X)
    u=((th+0.05*np.sin(t*0.3))/(2*np.pi))%1.0
    _u0=(th/(2*np.pi))%1.0
    v=(0.35/r + t*0.07)                     # scroll LENTO y CONSTANTE (aprobado)
    ui=((u*BTW*1.0)%BTW).astype(int)
    vi=((v*BTH*0.9)%BTH).astype(int)
    wall=base[vi,ui]
    vi2=((v*BTH*2.3+30)%BTH).astype(int); ui2=((u*BTW*2.0)%BTW).astype(int)
    wall=wall*0.7+base[vi2,ui2]*0.3
    wall=np.clip((wall-0.5)*1.3+0.5,0,1)
    depth=np.clip(r*1.7,0.12,1)
    img=wall*depth*1.7+wall*0.15
    img=gaussian_filter(img,0.8*S)          # blur escalado
    img=np.clip(img,0,1.6)/1.6
    deep=np.array([0.02,0.06,0.035]); brite=np.array([0.16,0.55,0.28])
    rgb=np.clip(deep+(brite-deep)*img[...,None],0,1)*255
    g=grim(rgb.astype(np.uint8), scale=float(S), grain_amt=2.0, gamma=1.32).astype(float)  # scale=S
    core=gaussian_filter(np.exp(-(r/0.11)**2),22*S)   # sigmas escalados
    halo=gaussian_filter(np.exp(-(r/0.26)**2),30*S)
    core/=core.max()+1e-9; halo/=halo.max()+1e-9
    g=g+core[...,None]*np.array([0.85,1.0,0.92])*255*0.85 + halo[...,None]*np.array([0.45,0.7,0.55])*255*0.6
    return np.clip(g,0,255).astype(np.uint8)
import glob
OUT='/Users/emilianomettini/outbound_work/tunel_np_1080'
os.makedirs(OUT,exist_ok=True)
for o in glob.glob(OUT+'/f*.png'): os.remove(o)
for f in range(3312):
    Image.fromarray(frame(f/24.0)).save(OUT+'/f%04d.png'%f)
    if f%200==0: print('tunel1080',f)
print('TUNEL 1080 DONE 3312')
