...
# BELL events del video ORIGINAL (destello de luz interior, SUTIL)
...
raw=sorted(glob.glob('/tmp/anim/full2/ovulo/raw/f*.png'), key=fnum)
out='/tmp/anim/full2/ovulo/grim'; os.makedirs(out,exist_ok=True)
...
for fp in raw:
    ...
    if H is None:
        H,W,_=arr.shape; cx,cy=W*0.499,H*0.498  # centro GEOM del disco (bounding box) = centro pantalla
        yy,xx=np.mgrid[0:H,0:W].astype(float)
        R=np.sqrt(((xx-cx)/(H*0.42))**2+((yy-cy)/(H*0.42))**2)
    b=bump(t)
    # DESTELLO de campanita SUTIL y TODO DIFUSO (sin nucleo duro): solo halo blureado
    if b>0.001:
        halo=gaussian_filter(np.exp(-(R/0.40)**2)*b,12.0)   # difuso fuerte, sin core punzante
        arr=arr+halo[...,None]*np.array([0.5,1.0,0.62])*255*1.0
    g=grim(np.clip(arr,0,255).astype(np.uint8), scale=1.0, bright=1.15, grain_amt=1.3, gamma=1.12)
    Image.fromarray(g).save(f'{out}/f{f:03d}.png')
print('OVULO POST DONE', len(raw))
