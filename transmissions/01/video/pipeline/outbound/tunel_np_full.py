...
BTH,BTW=base.shape
def frame(t):
    # SIN zoom de entrada: el zoom agregaba movimiento solo al principio (acelerado) y luego paraba.
    # Velocidad CONSTANTE = solo el scroll, parejo de punta a punta.
    X=X0; Y=Y0
    r=np.sqrt(X*X+Y*Y)+1e-4; th=np.arctan2(Y,X)
    u=((th+0.05*np.sin(t*0.3))/(2*np.pi))%1.0   # rotacion casi nula (no lavarropas)
    _u0=(th/(2*np.pi))%1.0                 # angular (pared alrededor)
    v=(0.35/r + t*0.07)                  # scroll LENTO y CONSTANTE (un toque mas lento aun)
...
import glob
os.makedirs('/Users/emilianomettini/outbound_work/tunel_np',exist_ok=True)
for o in glob.glob('/Users/emilianomettini/outbound_work/tunel_np/f*.png'): os.remove(o)
for f in range(3312):
    Image.fromarray(frame(f/24.0)).save('/Users/emilianomettini/outbound_work/tunel_np/f%04d.png'%f)
    if f%400==0: print('tunel',f)
print('TUNEL NP DONE 3312')
